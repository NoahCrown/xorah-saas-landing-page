                ...
                <div className='relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3'>
                    {features.map(({id, icon, caption, title, text, button}) => (
                        ...
                        key={id}>
                            ...
                            <p className='caption mb-5 max-md:mb-6 max-md:h-5'>
                                {caption}
                            </p>
                    ...
                    <ul className='relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl'>
                        <div className='absolute bg-s3/30 top-[38%] left-0 right-0 h-[1px] w-full'/>
                        {details.map(({ id, icon, title }) => (
                            <li key={id} className="relative pt-16 px-4 pb-14">
                                <div className='absolute top-0 bottom-0 left-1/2 bg-s3/20 w-[1px] h-full z-10'/>
                                <div className='flex item-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full hover:border-s4 transition-all duration-500 shadow-500 size-20'>
